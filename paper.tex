...
\begin{abstract}
Network intrusion detection remains a critical challenge in cybersecurity, particularly when dealing with highly imbalanced, sparse packet-level data. In this paper, we present an end-to-end pipeline for network intrusion detection that combines extensive feature engineering from raw packet captures with graph neural network (GNN) classification. We introduce LSGNN-DualTask, a novel extension of the Local Similarity Graph Neural Network (LSGNN) that augments the standard node classification objective with an auxiliary edge consistency loss. This dual-task formulation encourages the model to learn embeddings where connected nodes of the same attack type are clustered tightly, while sharpening the boundary between normal and malicious traffic. We evaluate our approach on a real-world labeled packet capture dataset containing 13{,}514 network flows across five traffic classes with severe class imbalance (713:1 ratio). Experiments demonstrate that LSGNN-DualTask consistently improves over the baseline LSGNN across all metrics (accuracy: 0.9686 vs.\ 0.9615; macro F1: 0.8332 vs.\ 0.7954), with the largest gains observed on the rarest attack class (+0.15 F1). We also provide a systematic comparison with strong tabular baselines (Random Forest, MLP) and analyze the contribution of each pipeline component.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Severe class imbalance}: Real-world attack datasets exhibit extreme imbalance, with rare attack types comprising less than 0.1\% of samples.
...
\begin{enumerate}
    ...
    \item We conduct a systematic comparison of four model families (Random Forest, MLP, LSGNN, LSGNN-DualTask) on a real-world dataset with five attack types and severe class imbalance.
    \item We demonstrate that the dual-task formulation improves the baseline LSGNN by +3.78\% average per-class F1, with the largest gains on the rarest attack class (+15\% F1).
\end{enumerate}
...
The combined graph for our dataset contains 195{,}608 edges (177{,}346 from k-NN, 28{,}276 from communication patterns) with an average node degree of 14.5 and edge homophily of 0.962.
...
\begin{enumerate}
    ...
    \item The boundary between normal and attack traffic is sharpened, improving detection of rare attacks that might otherwise be absorbed by the dominant class.
...
We evaluate on a real-world labeled packet capture dataset (\texttt{excel\_labeling\_complet.csv}) containing 13{,}514 network flows captured over approximately 485 seconds from a controlled network environment. The dataset comprises 9 raw columns: \texttt{packet\_id}, \texttt{timestamp}, \texttt{label}, \texttt{src\_ip}, \texttt{dst\_ip}, \texttt{protocol}, \texttt{src\_port}, \texttt{dst\_port}, and \texttt{length}.

The traffic is labeled into five classes representing stages of a lateral movement cyber attack campaign:
\begin{itemize}
    \item \textbf{BENIGNE} (3{,}176 samples, 23.5\%): Normal network traffic.
    \item \textbf{Copy 54ndc47 (SMB)} (9{,}281, 68.7\%): SMB-based file copy attack.
    \item \textbf{Service Creation} (578, 4.3\%): Remote service creation for persistence.
    \item \textbf{Discover local hosts} (466, 3.4\%): Network discovery/scanning.
    \item \textbf{View remote shares} (13, 0.1\%): SMB share enumeration.
\end{itemize}

The dataset exhibits severe class imbalance with a 713:1 ratio between the largest and smallest classes. Additionally, 245 rows (1.8\%) contain missing IP/protocol values, corresponding to broadcast or local packets. The network topology involves 12 unique source IPs and 14 unique destination IPs forming 28 unique communication pairs.
...
To address class imbalance, we apply SMOTE (Synthetic Minority Oversampling Technique)~\cite{chawla2002smote} to the training set. For classes with fewer than 6 samples (below the SMOTE $k$-neighbors threshold), we first apply RandomOverSampler to reach 6 samples, then SMOTE to balance minority classes to 30\% of the majority class count. This augments the training set from 8{,}108 to 12{,}483 samples while preserving the original validation and test sets.

\subsection{Data Splits}

We use stratified splitting with a 60/20/20 train/validation/test ratio, yielding 8{,}108 training, 2{,}703 validation, and 2{,}703 test samples. For classes with very few samples, we fall back to non-stratified splitting to avoid errors.
...
Table~\ref{tab:results} presents the overall performance of all four models on the test set. Random Forest achieves the highest accuracy (0.9985) and macro F1 (0.9554), demonstrating the strength of well-engineered tabular features combined with ensemble methods. Among neural models, LSGNN-DualTask outperforms both the MLP and the baseline LSGNN on accuracy and weighted F1, confirming the benefit of graph-based modeling with edge consistency regularization.
...
\midrule
Random Forest     & \textbf{0.9985} & \textbf{0.9554} & \textbf{0.9985} \\
MLP               & 0.9652          & 0.8861          & 0.9651          \\
LSGNN             & 0.9615          & 0.7954          & 0.9627          \\
LSGNN-DualTask    & 0.9686          & 0.8332          & 0.9696          \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Random Forest     & 0.9310 & \textbf{0.9932} & \textbf{1.0000} & \textbf{1.0000} \\
MLP               & 0.8480 & 0.9624          & 0.9882          & 0.9914          \\
LSGNN             & 0.7653 & 0.9596          & 0.9807          & 0.9818          \\
LSGNN-DualTask    & 0.7903 & 0.9709          & 0.9889          & 0.9944          \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:perclass} presents per-class F1 scores. The most notable finding is the performance on the rarest class, ``View remote shares'' (only 13 total samples, 2 in the test set). LSGNN-DualTask achieves an F1 of 0.4000 compared to 0.2500 for the baseline LSGNN---a 60\% relative improvement. While Random Forest still leads (0.8000 F1), the dual-task formulation substantially narrows the gap. This improvement is directly attributable to the edge consistency loss, which encourages the model to embed rare-class nodes distinctly from their neighbors.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Class} & \textbf{RF} & \textbf{MLP} & \textbf{LSGNN} & \textbf{DualTask} \\
\midrule
BENIGNE              & 0.998 & 0.931 & 0.926 & 0.942 \\
Copy 54ndc47 (SMB)   & 1.000 & 0.982 & 0.982 & 0.986 \\
Discover local hosts & 0.984 & 0.968 & 0.933 & 0.968 \\
Service Creation     & 0.996 & 0.883 & 0.886 & 0.870 \\
View remote shares   & 0.800 & 0.667 & 0.250 & 0.400 \\
\bottomrule
\end{tabular}
\end{table}

For the ``Discover local hosts'' class, LSGNN-DualTask recovers the full performance of the MLP baseline (0.968 F1), substantially improving over the baseline LSGNN (0.933). This suggests that the edge consistency objective helps the GNN leverage graph structure more effectively for minority classes that benefit from neighbor-based reasoning.

\subsection{LSGNN vs.\ LSGNN-DualTask}

Table~\ref{tab:improvement} summarizes the per-class improvements of LSGNN-DualTask over the baseline LSGNN. On average, the dual-task modification improves per-class F1 by +0.0378. Improvements are observed on 4 out of 5 classes, with the only regression occurring on ``Service Creation'' ($-$0.016 F1). The average improvement demonstrates that the edge consistency regularization provides a consistent benefit across most traffic types.
...
\midrule
BENIGNE              & +0.016 & $\uparrow$ \\
Copy 54ndc47 (SMB)   & +0.004 & $\uparrow$ \\
Discover local hosts & +0.035 & $\uparrow$ \\
Service Creation     & $-$0.016 & $\downarrow$ \\
View remote shares   & +0.150 & $\uparrow\uparrow$ \\
\midrule
\textbf{Average}     & \textbf{+0.038} & $\uparrow$ \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Why does Random Forest outperform GNNs?} The strong performance of Random Forest can be attributed to several factors. First, our feature engineering creates a rich 68-dimensional representation that already captures many of the relational and temporal patterns that GNNs learn implicitly. Second, Random Forest is inherently robust to feature scale and noise, and benefits directly from SMOTE augmentation on the tabular feature space. Third, the dataset's high edge homophily (0.962) means that most graph structure is already captured by feature similarity, limiting the additional information that GNNs can extract from explicit message passing.

\textbf{Value of the dual-task objective.} Despite GNNs not surpassing the tabular baseline overall, the dual-task modification demonstrates a clear and consistent improvement over the baseline LSGNN. The edge consistency loss is particularly effective for rare classes, where the auxiliary signal helps prevent the model from collapsing rare-class embeddings into the dominant class. This is evidenced by the +0.15 F1 improvement on ``View remote shares,'' the rarest class with only 13 total samples.

\textbf{Graph structure analysis.} The constructed graph has high homophily (0.962), meaning 96.2\% of edges connect nodes of the same class. This benefits standard message passing but also means heterophilic edges (connecting different classes) are informationally rich. The LSGNN's similarity-weighted message passing can selectively attend to these boundary edges, and the edge consistency loss further encourages the model to correctly identify cross-class connections.
...
We presented LSGNN-DualTask, an end-to-end pipeline for network intrusion detection that combines extensive feature engineering from sparse packet-level data with a dual-task graph neural network. Our edge consistency regularization objective improves the baseline LSGNN by an average of +3.78\% per-class F1, with the largest gains on the rarest attack class (+15\% F1). The complete pipeline achieves strong performance across all models, with the Random Forest baseline reaching 0.9985 accuracy and the GNN models providing complementary graph-based reasoning capabilities.

Our systematic comparison reveals that well-engineered tabular features remain highly competitive for cybersecurity classification, but GNNs offer advantages in modeling relational structure and can be further improved with task-relevant auxiliary objectives. The dual-task formulation is general and can be applied to any GNN architecture without modifying its core message-passing mechanism.
