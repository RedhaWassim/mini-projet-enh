...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{BENIGNE}: Normal, legitimate traffic.
    \item \textbf{Copy 54ndc47 (SMB)}: An SMB-based file copy attack (majority class).
    \item \textbf{Service Creation}: Remote service creation for persistence.
    \item \textbf{Discover local hosts}: Network scanning/discovery.
    \item \textbf{View remote shares}: SMB share enumeration (rarest class).
\end{enumerate}

The challenge is that our raw data has only 9~columns (\texttt{packet\_id}, \texttt{timestamp}, \texttt{label}, \texttt{src\_ip}, \texttt{dst\_ip}, \texttt{protocol}, \texttt{src\_port}, \texttt{dst\_port}, \texttt{length}), the classes are extremely imbalanced, and the attacks represent sequential stages of a lateral movement campaign. We tackle this with a pipeline that has six stages, each building on the previous one.
...
Our dataset exhibits severe class imbalance: the majority class dominates the distribution while the rarest class has only a handful of samples. A model that always predicts the majority class would achieve high accuracy but would completely fail to detect rare attacks. This is called \emph{class imbalance}.
...
For extremely rare classes, SMOTE cannot work directly because it needs $k+1 = 6$ same-class neighbors. Our solution is a two-step approach:
...
This significantly augments the training set size. Importantly, \emph{only the training set is augmented}---validation and test sets remain untouched to ensure unbiased evaluation.
...
The k-NN construction produces the majority of edges in our graph.
...
After merging k-NN and communication-based edges with deduplication, the resulting graph has a high average node degree, ensuring rich neighborhood information for message passing.
...
The LSGNN parameter budget is distributed across:
\begin{itemize}
    \item Input projection: $\mat{W}_{\text{proj}} \in \R^{d_h \times d_{\text{in}}}$ with bias.
    \item Layer normalization: $\bm{\gamma}, \bm{\beta} \in \R^{d_h}$.
    \item Per LSGNN layer ($\times L$ layers):
    \begin{itemize}
        \item Similarity MLP: $\mat{W}_1 \in \R^{d_h \times 3d_h}$, $\mat{W}_2 \in \R^{1 \times d_h}$ with biases.
        \item Message transform: $\mat{W}_{\text{msg}} \in \R^{d_h \times d_h}$.
        \item Update: $\mat{W}_{\text{upd}} \in \R^{d_h \times 2d_h}$ with bias.
        \item LayerNorm: $\bm{\gamma}, \bm{\beta} \in \R^{d_h}$.
    \end{itemize}
    \item Classifier: Two-layer MLP with hidden dimension $d_h / 2$.
\end{itemize}
The total model is lightweight, with the similarity MLPs accounting for the largest share of parameters.
...
\begin{enumerate}
    ...
    \item \textbf{Helps rare classes}: for classes with very few samples, the node loss provides very few gradient signals. The edge loss provides additional signal every time an edge touches a rare-class node.
\end{enumerate}
...
Computing the edge loss over all edges is expensive. We sample 50\% of edges randomly each training step:
\begin{equation}
...
LSGNN-DualTask adds only the edge consistency head ($\mat{W}_3 \in \R^{(d_h/2) \times d_h}$ and $\mat{W}_4 \in \R^{1 \times (d_h/2)}$ with biases), representing less than 4\% additional parameters over the baseline LSGNN. This is a very lightweight extension.
...
\end{equation}
Simple and interpretable, but misleading with imbalanced data (a model predicting only the majority class achieves high accuracy while missing all attacks).
...
\begin{table}[t]
\centering
\caption{Per-Class F1 Scores and Improvement from Dual-Task}
...
\midrule
\textbf{Macro Avg.}  & .955 & .886 & .795 & .833 & +.038 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Per-Class Precision Analysis}

Precision measures the fraction of predicted positives that are actually positive. Table~\ref{tab:perclass_prec} shows per-class precision for all models.

\begin{table}[t]
\centering
\caption{Per-Class Precision Scores}
\label{tab:perclass_prec}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Class} & \textbf{RF} & \textbf{MLP} & \textbf{LSGNN} & \textbf{Dual} \\
\midrule
BENIGNE              & .997 & .960 & .988 & .975 \\
Copy 54ndc47         & 1.00 & .970 & .955 & .963 \\
Discover local hosts & .969 & .938 & .875 & .938 \\
Service Creation     & 1.00 & .866 & .879 & .870 \\
View remote shares   & .667 & .500 & .143 & .250 \\
\midrule
\textbf{Macro Avg.}  & .931 & .848 & .765 & .790 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{View remote shares} precision improves from 0.143 (LSGNN) to 0.250 (Dual-Task), meaning the model produces fewer false positives for this rare class. The LSGNN baseline frequently mislabels benign packets as this attack type; the edge consistency loss tightens the decision boundary.
    \item \textbf{BENIGNE} precision slightly decreases from 0.988 to 0.975, a trade-off: the model becomes slightly more liberal in predicting benign traffic in order to correctly capture more minority-class samples.
    \item Random Forest achieves near-perfect precision on most classes, benefiting from its ensemble voting mechanism and balanced class weights.
\end{itemize}

\subsection{Per-Class Recall Analysis}

Recall measures the fraction of actual positives that are correctly detected. For intrusion detection, recall is critical---a missed attack (false negative) can be catastrophic.

\begin{table}[t]
\centering
\caption{Per-Class Recall Scores}
...
Discover local hosts & 1.00 & 1.00 & 1.00 & 1.00 \\
Service Creation     & .992 & .901 & .893 & .869 \\
View remote shares   & 1.00 & 1.00 & .500 & 1.00 \\
\midrule
\textbf{Macro Avg.}  & .993 & .962 & .960 & .971 \\
\bottomrule
\end{tabular}
\end{table}

Key recall observations:
\begin{itemize}
    \item \textbf{View remote shares} recall jumps from 0.500 (LSGNN) to 1.000 (Dual-Task). The baseline LSGNN misses half of the rare attack samples; the edge consistency loss ensures the model learns to detect all instances by leveraging neighborhood signals from connected attack nodes.
    \item \textbf{BENIGNE} recall improves from 0.873 to 0.912 (+4.5\%). The dual-task objective helps the model better distinguish normal traffic from attack patterns.
    \item \textbf{Copy 54ndc47} and \textbf{Discover local hosts} maintain perfect recall (1.000) across both GNN variants, indicating these patterns are well-captured by the graph structure.
    \item \textbf{Macro recall} improves from 0.960 to 0.971, confirming that the dual-task approach broadly improves detection rates.
\end{itemize}

\subsection{Precision--Recall Trade-off Discussion}

The results reveal the classic \emph{precision--recall trade-off}. LSGNN-DualTask improves recall significantly (especially for rare classes) at a modest cost to precision on some classes. For network intrusion detection, this trade-off is generally favorable: it is far more costly to \emph{miss} an attack (low recall) than to generate a few extra alerts that require analyst review (lower precision). The edge consistency loss effectively shifts the operating point toward higher recall by ensuring that nodes connected to known attack nodes are more likely to be flagged.
...
\begin{enumerate}
    ...
    \item With high edge homophily, most graph-structural information is redundant with feature similarity---the k-NN graph is built from the same features.
\end{enumerate}
